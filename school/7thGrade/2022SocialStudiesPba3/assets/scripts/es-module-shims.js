...
(function () {
  ...
  const esmsInitOptions = optionsScript
  ...
  let fetchHook = esmsInitOptions.fetch
    ? globalHook(esmsInitOptions.fetch)
    : fetch;
  const metaHook = esmsInitOptions.meta
    ? globalHook(shimModule && esmsInitOptions.meta)
    : noop;
  ...
  const { revokeBlobURLs, noLoadEventRetriggers, enforceIntegrity } =
    esmsInitOptions;
  ...
  const throwError = (err) => {
    (window.reportError || (window.safari && console.error) || eoop)(err),
      void onerror(err);
  };
  ...
  function resolveIfNotPlainOrUrl(relUrl, parentUrl) {
    ...
    const queryHashIndex = parentUrl.indexOf(
      "?",
      parentUrl.indexOf("#") === -1 ? parentUrl.indexOf("#") : parentUrl.length
    );
  ...
  function parse(k, l = "@") {
                      ...
                      case 41: {
                        ...
                        if (
                          (r << 16) >> 16 != 0
                            ? ((w =
                                t[
                                  ((t[60] | 0) + (((r & 65535) + -1) << 2)) >> 2
                                ] | 0),
                              (t[(w + 20) >> 2] | 0) ==
                                (t[((t[59] | 0) + ((a & 65535) << 2)) >> 2] |
                                  0))
                            : 0
                        ) {
                          a = (w + 4) | 0;
          ...
          function k() {
            ...
            e: do {
              if ((c | 0) == 3) {
                ...
                e = t[61] | 0;
                if (
                  (a << 16) >> 16 == 102 ? (m((e + 2) | 0, 52, 6) | 0) == 0 : 0
                ) {
                  t[61] = e + 8;
                  u(f, w(1) | 0);
                  break;
    ...
    for (; e.ri(); ) {
      ...
      let n;
      e.ip() &&
        (n = b(-1 === i ? a : a + 1, c$1.charCodeAt(-1 === i ? a - 1 : a))),
        w.push({ n: n, s: a, e: r, ss: t, se: f, d: i, a: s });
    }
    for (; e.re(); ) {
      const a = e.es(),
        r = c$1.charCodeAt(a);
  ...
  function k() {
    ...
    switch ((++n, e)) {
      ...
      case 117:
        return (function () {
          let e;
          123 === c$1.charCodeAt(n)
            ? (++n, (e = l(c$1.indexOf("}", n) - n)), ++n, e > 1114111 && o())
            : (e = l(4));
          return e <= 65535
  ...
  function o() {
    throw Object.assign(
      Error(
        `Parse error ${f}:${c$1.slice(0, n).split("\n").length}:${
          n - c$1.lastIndexOf("\n", n - 1)
        }`
      ),
      { idx: n }
  ...
  async function doFetch(url, fetchOpts, parent) {
    ...
    if (!res.ok)
      throw Error(
        `${res.status} ${res.statusText} ${res.url}${fromParent(parent)}`
      );
    return res;
  }
